...
public enum ApiKeyword {
     ...
     * 표시명으로 키워드 찾기
     */
    public static ApiKeyword fromDisplayName(String displayName) {
        for (ApiKeyword keyword : values()) {
            if (keyword.displayName.equals(displayName)) {
                return keyword;
            }
        }
        return null;
     ...
     * 특정 도메인의 키워드 개수 반환
     */
    public static int getKeywordCountByDomain(ApiDomain domain) {
        return (int) java.util.Arrays.stream(values())
            .filter(keyword -> keyword.domain == domain)
            .count();
    }
    ...
    public static String[] getAllCodes() {
        ApiKeyword[] keywords = values();
        String[] codes = new String[keywords.length];
        for (int i = 0; i < keywords.length; i++) {
            codes[i] = keywords[i].code;
        }
        return codes;
    }

    /**
     * 특정 도메인의 키워드 코드 목록 반환
     */
    public static String[] getCodesByDomain(ApiDomain domain) {
        ApiKeyword[] keywords = getKeywordsByDomain(domain);
        String[] codes = new String[keywords.length];
        for (int i = 0; i < keywords.length; i++) {
            codes[i] = keywords[i].code;
        }
    ...
    public String toString() {
        return String.format("%s(%s, %s, %s)", name(), code, displayName, domain.getCode());
    }
}
