...
public enum ApiDomain {
     ...
     * 표시명으로 도메인 찾기
     */
    public static ApiDomain fromDisplayName(String displayName) {
        for (ApiDomain domain : values()) {
     ...
     * 한국어 표시명으로 도메인 찾기
     */
    public static ApiDomain fromKoreanName(String koreanName) {
        for (ApiDomain domain : values()) {
            if (domain.displayName.equals(koreanName)) {
                return domain;
            }
        }
        return OTHERS; // 기본값
    ...
    public static String[] getAllCodes() {
        ApiDomain[] domains = values();
        String[] codes = new String[domains.length];
        for (int i = 0; i < domains.length; i++) {
            codes[i] = domains[i].code;
        }
        return codes;
    }
